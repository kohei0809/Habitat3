...
class HabitatSimInteractiveViewer(Application):
    ...
    def move_and_look(self, repetitions: int) -> None:
        ...
        if self.is_stop == False:
            ...
            print("------------------")
            print(f"agent: x={self.follower._follower.agent.state}")
            #forward
            if best_action == 0:
                new_x = self.kin_humanoid1.base_pos[0] + 0.05*math.sin(self.kin_humanoid1.base_rot+1.57)
                new_y = self.kin_humanoid1.base_pos[2] + 0.05*math.cos(self.kin_humanoid1.base_rot+1.57)
                self.kin_humanoid1.base_pos = mn.Vector3(new_x, self.kin_humanoid1.base_pos[1], new_y)
                #self.follower._follower.agent.state.position = np.array([new_x, self.kin_humanoid1.base_pos[1], new_y])
                #print("YYY")
                #print(self.follower._follower.agent.agent_config.action_space)
                self.follower._follower.agent.act("move_forward")
                #print(f"agent: x={self.follower._follower.agent.state.position}")
                #self.follower._follower._move_forward()
                
                
            #turn-left
            elif best_action == 1:
                self.kin_humanoid1.base_rot += math.pi/12
                #self.follower._follower.agent.state.rotation = self.kin_humanoid1.base_rot
                self.follower._follower.agent.act("turn_left")
                
            #turn-right
            elif best_action == 2:
                self.kin_humanoid1.base_rot -= math.pi/12
                #self.follower._follower.agent.state.rotation = self.kin_humanoid1.base_rot
                self.follower._follower.agent.act("turn_right")
